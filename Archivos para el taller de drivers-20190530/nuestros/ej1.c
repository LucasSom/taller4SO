...
//el contrato de read es devolver la cantidad que pudo leer.
ssize_t read_nuestro(struct file* archivo, char __user* buffer_usuario, size_t tamanio_buffer, loff_t* offset){
	ssize_t algo=0;
	//le digo que hay cero cosas en el archivo.
	return algo;
}

//el contrato de write es devolver la cantidad de caracteres que llegue a copiar
ssize_t write_nuestro(struct file* archivo, const char __user* buffer_usuario, size_t tamanio_buffer, loff_t* offset){
	ssize_t algo=tamanio_buffer;
	//le digo que copie todo el tamanio del buffer para que se quede pancho el user
	return algo;
}
